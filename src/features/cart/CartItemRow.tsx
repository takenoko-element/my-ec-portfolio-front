...
const CartItemRow = ({item}: CartItemProps) => {
    ...
    const handleIncrement = () => {
        updateQuantity(
            ...
            {
                onError: (error: Error) => toast.error(error.message || '数量の更新に失敗しました。'),
            }
        )
    }
    ...
    const handleRemoveItem = () => {
        removeItem(
            item.id,
            {
                onSuccess: () => toast.success(`「${item.product.title}」をカートから削除しました。`),
                onError: (error: Error) => toast.error(error.message || 'アイテムの削除に失敗しました。'),
            }
        )
    }
        ...
        >
            ...
            <div className="flex-grow text-center sm:text-left">
                <h3 className="text-lg font-semibold text-gray-800 mb-1">
                    <Link to={`/product/${item.productId}`} className="hover:text-blue-600 hover:underline">{item.product.title}</Link>
                </h3>
